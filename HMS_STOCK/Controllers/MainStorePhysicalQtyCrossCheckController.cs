...
namespace HMS_STOCK.Controllers
{
    [SessionExpire]
    [RoutePrefix("Reports")]
    public class MainStorePhysicalQtyCrossCheckController : Controller
    {
        private readonly ApplicationDbContext db = new ApplicationDbContext();

        [HttpGet]
        [Route("MainStorePhysicalQtyCrossCheck")]
        ...
        [HttpGet]
        [Route("MainStorePhysicalQtyCrossCheck/DownloadPdf")]
        public ActionResult DownloadPdf(int? materialGroupId, string from, string to)
        {
            ...
            const string viewName = "VW_NEW_DRUG_PHY_QTY_ASSGN_2627";
...
WHERE 1 = 1";
            ...
            {
                title = "MAIN STORE " + materialGroupName.Trim() + " PHYSICAL QTY CROSS CHECK";
                if (isTabletsGroup && alphaFrom != null && alphaTo != null)
                {
                    title = "MAIN STORE " + materialGroupName.Trim() + " (" + alphaFrom + " - " + alphaTo + ") PHYSICAL QTY CROSS CHECK";
                }
            }
