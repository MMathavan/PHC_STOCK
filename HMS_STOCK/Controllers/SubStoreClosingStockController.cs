...
{
    public class SubStoreClosingStockController : Controller
    {
        ...
        public ActionResult Index(int? materialGroupId)
        {
            ...
            if (materialGroupId.HasValue && materialGroupId.Value > 0)
            {
                totalQty = db.Database.SqlQuery<decimal>(
                    "SELECT ISNULL(SUM(MTRLSTKQTY), 0) FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0 AND MTRLGID = @p0", 
                    materialGroupId.Value).FirstOrDefault();
                    
                totalValue = db.Database.SqlQuery<decimal>(
                    "SELECT ISNULL(SUM(CLVALUE), 0) FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0 AND MTRLGID = @p0", 
                    materialGroupId.Value).FirstOrDefault();
            }
            else
            {
                totalQty = db.Database.SqlQuery<decimal>(
                    "SELECT ISNULL(SUM(MTRLSTKQTY), 0) FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0").FirstOrDefault();
                    
                totalValue = db.Database.SqlQuery<decimal>(
                    "SELECT ISNULL(SUM(CLVALUE), 0) FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0").FirstOrDefault();
            }
        ...
        public JsonResult GetStockData(DataTableRequest request)
        {
            try
            {
                ...
                var sql = @"DECLARE @TotalRowsCount int, @FilteredRowsCount int;
                    EXEC [dbo].[pr_substoreSearchclosingstock]
                        @FilterTerm = {0},
                        @SortIndex = {1},
                        @SortDirection = {2},
                        @StartRowNum = {3},
                ...
                if (hasMaterialFilter)
                {
                    ...
                    var query = "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY " + orderByClause + ") AS RowNum FROM SubStoreStockMaster_2526 " + where + ") AS T WHERE T.RowNum BETWEEN @p" + (hasSearch ? "2" : "1") + " AND @p" + (hasSearch ? "3" : "2") + ";";
                ...
                {
                    var query = "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY " + orderByClause + ") AS RowNum FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0 AND (MTRLDESC LIKE @p0 OR BATCHNO LIKE @p0 OR CONVERT(varchar(10), STKEDATE, 23) LIKE @p0)) AS T WHERE T.RowNum BETWEEN @p1 AND @p2";
                ...
                {
                    stockData = db.Database.SqlQuery<SubStoreStockMaster_2526>(
                        "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY " + orderByClause + ") AS RowNum FROM SubStoreStockMaster_2526 WHERE ISNULL(STKBID, 0) <> 0) AS T WHERE T.RowNum BETWEEN @p0 AND @p1",
        ...
        public JsonResult GetRow(int stkBid)
        {
            try
            {
                var row = db.Database.SqlQuery<SubStoreStockMaster_2526>(
                        "SELECT TOP 1 * FROM SubStoreStockMaster_2526 WHERE STKBID = @p0",
                        stkBid)
                    .FirstOrDefault();
