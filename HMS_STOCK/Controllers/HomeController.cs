...
WHERE {physicalWhere}
  AND (
        ISNULL(LTRIM(RTRIM(CURRENTBATCH)), '') <> ''
     OR (PHYQTY IS NOT NULL AND PHYQTY <> 0)
...
ORDER BY ISNULL(MTRLGDESC, '')";
        ...
        [HttpGet]
        public ActionResult DownloadPharmacyDashboardPdf()
        {
            if (!IsDashboardUser())
            {
                return new HttpUnauthorizedResult();
            ...
            try
            {
                ...
                byte[] pdfBytes = BuildSingleDashboardPdf(dt, "PHARMACY OPENING CLOSING STOCK 2026 - 2027");
                return File(pdfBytes, "application/pdf", "PHARMACY_OPENING_CLOSING_STOCK_26-27.pdf");
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public ActionResult DownloadSubStoreDashboardPdf()
        {
            if (!IsDashboardUser())
            {
                return new HttpUnauthorizedResult();
            ...
            try
            {
                ...
                byte[] pdfBytes = BuildSingleDashboardPdf(subStoreDt, "SUB STORE OPENING CLOSING STOCK 2026 - 2027");
                return File(pdfBytes, "application/pdf", "SUB_STORE_OPENING_CLOSING_STOCK_26-27.pdf");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = ex.Message;
        ...
        private static byte[] BuildSingleDashboardPdf(DataTable dt, string heading)
        {
