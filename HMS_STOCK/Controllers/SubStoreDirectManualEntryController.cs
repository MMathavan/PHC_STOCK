...
{
    public class SubStoreDirectManualEntryController : Controller
    {
        ...
        public JsonResult GetManualEntryData(DataTableRequest request)
        {
            try
            {
                ...
                int totalCount = db.Database.SqlQuery<int>(
                    "SELECT COUNT(*) FROM SubStoreStockMaster_2526 WHERE STKBID = 0").FirstOrDefault();

                int filteredCount = totalCount;
                List<SubStoreStockMaster_2526> rows;

                bool hasSearch = !string.IsNullOrWhiteSpace(searchValue);

                if (hasSearch)
                {
                    var query = "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY " + sortColumnName + " " + sortDirection + ") AS RowNum FROM SubStoreStockMaster_2526 WHERE STKBID = 0 AND (MTRLDESC LIKE @p0 OR BATCHNO LIKE @p0 OR CONVERT(varchar(10), STKEDATE, 23) LIKE @p0)) AS T WHERE T.RowNum BETWEEN @p1 AND @p2";

                    rows = db.Database.SqlQuery<SubStoreStockMaster_2526>(
                        query,
                ...
                {
                    rows = db.Database.SqlQuery<SubStoreStockMaster_2526>(
                        "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY " + sortColumnName + " " + sortDirection + ") AS RowNum FROM SubStoreStockMaster_2526 WHERE STKBID = 0) AS T WHERE T.RowNum BETWEEN @p0 AND @p1",
        ...
        public ActionResult Edit(int id)
        {
            var row = db.Database.SqlQuery<SubStoreStockMaster_2526>(
                "SELECT TOP 1 * FROM SubStoreStockMaster_2526 WHERE SID = @p0", id).FirstOrDefault();
...
WHERE SID = @p8",
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                int affected = db.Database.ExecuteSqlCommand(
                    "DELETE FROM SubStoreStockMaster_2526 WHERE SID = @p0", id);
        ...
        public ActionResult Add(int? mtrlid, string currentBatch, decimal? phyQty, DateTime? expiryDate)
        {
            ...
            try
            {
                ...
                var sql =
                    @"INSERT INTO SubStoreStockMaster_2526
(
    STKBID,
    TRANREFID,
    TRANREFNAME,
